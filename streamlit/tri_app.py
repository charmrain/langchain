...
def load_multi_html():
    ...
    st.subheader("Multi-HTML Loader")
    input_url = st.text_input("Please enter the URL containing multiple HTML links:")
    if input_url:
        # Check if the URL contains a scheme, if not, add 'http://'
        ...
        try:
            ...
            # Extract all URLs from the input URL
            links = soup.find_all('a', href=True)
            all_urls = [urljoin(input_url, link['href']) for link in links]


            #  # Filter out potentially useless URLs
            filtered_urls = []
            for url in all_urls:
                # Exclude URLs containing javascript
                if not re.match(r'^javascript:', url, re.IGNORECASE):
                    # Add more exclusion criteria as needed
                    filtered_urls.append(url)

            # Display the list of extracted URLs for user selection
            selected_urls = st.multiselect("Select URLs to extract text from:", filtered_urls)
            ...
            for url in selected_urls:
                try:
                    ...
                    sub_main_text = soup.get_text()
                    main_text += sub_main_text

                    # Generate a unique name for each document based on the URL
                    valid_chars = "-_.() %s%s" % (string.ascii_letters, string.digits)
                    url_filename = ''.join(c if c in valid_chars else '_' for c in url)
                    pdfname_list.append(url_filename)
                except Exception as e:
                    st.write(f"Error loading HTML content from the provided URL {url}: {str(e)}")

            if main_text:
                st.write("Data loaded from selected HTML URLs:")
                return main_text, pdfname_list
            else:
                st.write("No valid HTML content loaded.")
                return None, None

        except Exception as e:
            st.write(f"Error extracting URLs from the input URL: {str(e)}")
            return None, None

    else:
        st.write("Please enter a valid URL containing multiple HTML links.")
        return None, None
...
with st.sidebar:
    ...
    st.image('https://i.ibb.co/N9ZsgB8/performix.png', use_column_width=True)
    st.markdown('''
    ## About
...
def main():
    try:
        ...
        load_dotenv()
        options_table = pd.DataFrame({'Loader Type': ['PDF', 'HTML','Multiple URLs']})
        ...
        elif loader_type == 'Multiple URLs':
            main_text, pdfname= load_multi_html()
